...
- **Server URLs nay align xuyên suốt** (P0.5): tất cả component dùng helper chung `agent/shared/server_urls.py::collect_server_urls(config, allow_dev_default=False)`. Empty config ⇒ empty list ⇒ component vào OFFLINE mode (skip silently). Không còn fallback ngầm `http://localhost:5000` ở `manager.py:95`, `heartbeat.py:57`, `sender.py:54`. Chỉ caller nào pass `allow_dev_default=True` mới được dev default — production tuyệt đối không.
