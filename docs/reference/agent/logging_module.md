...
| `._get_server_urls(config)` | `(Dict) -> List[str]` | [sender.py:43](../../../agent/logging_module/sender.py#L43) | `server.urls` → `server.url` → fallback `["https://firewall-controller.onrender.com", "http://localhost:5000"]` |
...
- **Server URL fallback chứa `"https://firewall-controller.onrender.com"`** (sender.py:51) — hardcoded production URL. Nếu agent chạy ở môi trường khác chưa configure, sẽ POST log lên Render production. **Đáng cảnh giác**. Sửa nếu thấy không phù hợp.
