...
| `GET`  | `/api/agents/debug/status` | `debug_status` | login | — | Debug snapshot |
| `GET`  | `/api/agents/debug/direct` | `debug_direct_call` | login | — | Check reachable |
...
| `POST` | `/api/auth/refresh` | `AuthController.refresh_token` | — | — | Agent refresh JWT |
| `POST` | `/api/auth/logout` | `logout` | — | — | Revoke tokens |
| `POST` | `/api/auth/verify` | `verify_token` | — | — | Verify validity |
| `GET`  | `/api/auth/token-info` | `token_info` | — | — | Decode info |
| `POST` | `/api/admin/auth/login` | `AdminAuthController.login` | — | — | Admin/teacher login → set cookies |
...
- `create_api_key()` — POST `{name, description, expires_in_days, permissions}`. Status 201. `expires_in_days=0` → never expires
...
### `controllers/admin_auth_controller.py` — Admin/Teacher auth (`/api/admin/auth/*`)
[admin_auth_controller.py:28](../../../server/controllers/admin_auth_controller.py#L28)

| Constructor | `(admin_auth_service, jwt_service, socketio=None)` |
|---|---|

**Cookie constants** (line 20-25): `access_token` / `refresh_token`, httponly=True, secure=False (TODO production True), samesite=Lax.
...
- `update_profile()` — PUT `{email}`. `@require_login`. Email uniqueness check
...
- **AdminAuthController** chấp nhận token từ httpOnly cookie HOẶC body (refresh endpoint). Cookie ưu tiên. Cho phép cả SPA (cookie) lẫn mobile app (body). Hiện chưa có rate-limit ⇒ chỉ rely brute-force trong UserModel.
...
### Token cookies
- **`secure=False` mặc định** (admin_auth_controller.py:23). Production deploy HTTPS phải đổi `True` — hiện hardcoded. Nếu deploy không sửa, cookie có thể leak qua HTTP.
